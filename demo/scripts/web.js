...
app.controller('PostsCtrl',function($scope,$location,$anchorScroll){
  ...
  var addItem = function(){
    appLogic.addToDoItem(testToDoItems,$scope.itemBody,$scope.selectedList);
    $scope.itemBody = null;
  ...
  $scope.reOpenTask = function(selectedItemId){
    ...
    $scope.todoitems = appLogic.reOpenToDoItem(testToDoItems,selectedItemId);
	$scope.todoitems = testToDoItems.slice();
  };
  $scope.completeTask = function(selectedItemId){
    appLogic.completeToDoItem(testToDoItems,selectedItemId);
	$scope.todoitems = testToDoItems.slice();
  };
  $scope.removeTask = function(selectedItemId){
    appLogic.removeToDoItem(testToDoItems,selectedItemId);
	$scope.todoitems = testToDoItems.slice();
  };
  $scope.editTask = function(selectedItemId){
    $scope.doAction = function(){
        appLogic.editToDoItem(testToDoItems,selectedItemId,$scope.itemBody);
  ...
  $scope.LoadList = function(myListId){
	  ...
	  myListId = myListId?myListId:($scope.selectedList!=-1?myLists[$scope.selectedList].Id:false);
	  if(myListId){
